...
func TestSchemaContainsCompleteWideBookAndRequiredEngines(t *testing.T) {
	...
	for _, required := range []string{"bid_price_01 Int64", "bid_qty_50 UInt64", "ask_price_50 Int64", "ask_qty_50 UInt64", "ReplacingMergeTree(is_actual)", "funding_time DateTime64(3, 'UTC')", "MODIFY COLUMN funding_time DateTime64(3, 'UTC')", "yield_route", "yield_observation", "unbonding_seconds Nullable(UInt64)", "MODIFY COLUMN unbonding_seconds Nullable(UInt64)", "reward_lengths", "FINAL"} {
		if required == "FINAL" {
			continue
		}
